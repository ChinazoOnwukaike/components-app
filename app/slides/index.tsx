...
const SlideItem = ({ item }: SlideItemType) => {
  const { width } = useWindowDimensions();
  const { title, desc, img } = item;

  //can't dynamically find width within classname ex w-[${number}]. Must do in style
  return (
    <ThemedView
    ...
    >
      ...
      <ThemedText
        type="h1"
        className="text-light-primary dark:text-dark-primary px-10"
      >
        {title}
      </ThemedText>

      <ThemedText className="mt-10 px-10">{desc}</ThemedText>
    </ThemedView>
  );
};
const SlidesScreen = () => {
